...
//
// Analyse de la base pour extraire les tokens les plus frequents par variable de type texte
...
public:
	...
	// texte
	// Entree:
	// 	. sourceDatabase: base a analyser, correctement parametree avec uniquement des variables de type texte (Text ou TextList)
	// en used dans la classe principale
	// 	. ivTokenNumbers: nombre de tokens a extraire par variable de type texte, dans le meme ordre que les variables de type texte
	// Sortie:
	// 	. oaCollectedTokenSamples: tableau de samples de tokens (ObjectArray de KWTokenFrequency), dans le meme ordre que les variables de type texte
	// Methode interruptible, retourne false si erreur ou interruption (avec
	// message), true sinon
	// Memoire: le tableau en sortie appartient a l'appelant, et doit etre initialise a la bonne
	// taille, avec un tableau vide prepare par variable de type texte pour memoriser les tokens collectes.
...
protected:
	...
	// On effectue alors une seconde passe avec les tokens ainsi identifies, et on ne garde que
	...
	//   - la premiere passe collecte en flux un echantillon de tokens parmi les plus frequents
	//     pour gerer l'incertitude liee aux streams, on collecte plus de tokens que demande
	//   - la deuxieme passe prend ces tokens en entree et calcule de facon exacte leur effectif
	...
	// Les deux passes etant basees essentiellement sur la tokenisation des textes sont tres proches.
	// On choisit d'implementer ces deux taches dans la meme classe pour factoriser le code, en jouant
	// sur des variantes d'un parametrage en entree et en sortie.

	// Pilotage de l'ensemble des deux passes
	boolean InternalCollectTokenSamples(const KWDatabase* sourceDatabase);

	// Verification de la coherence des parametres de pilotage des deux passes
	...
	// Affichage des indicateurs collectes, en associant un libelle ainsi qu'un tableau de tokenizers
	...
	//////////////////////////////////////////////////////////////////////////////////////////
	// Implementation des methodes de KWDatabaseTask pour paralleliser la collecte des tokens
	...
	// Indicateur pour savoir si on est dans la premiere passe
	PLShared_Boolean shared_bIsFirstPass;

	// Activation du calcul des effectifs exacts des tokens, c'est a dire qu'une deuxieme passe est necessaire
	...
	// Tableau des tokens specifiques dont il faut calcul l'effectif, pour la deuxieme passe
	PLShared_ObjectArray* shared_oaSecondPassSpecificTokens;

	//////////////////////////////////////////////////////
	// Parametres en entree et sortie des esclaves

	// Tableau des tokens en sortie d'un esclave
	// Dans la premiere passe, il s'agit d'un echantillon de tokens collecte par l'esclave
	// Dans le seconde passe, il s'agit des tokens dont il faut calculer l'effectif, avec l'effectif
	...
	// Tableau de samples de tokens extraits par variable de type texte, copie du parametre principal en sortie
