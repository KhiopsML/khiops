...
MHDiscretizerHistogramMODL::MHDiscretizerHistogramMODL()
{
...
const ALString MHDiscretizerHistogramMODL::GetName() const
{
	return "HistogramMODL";
}

KWDiscretizer* MHDiscretizerHistogramMODL::Create() const
{
	return new MHDiscretizerHistogramMODL;
}

boolean MHDiscretizerHistogramMODL::IsUsingSourceValues() const
{
	return true;
}

void MHDiscretizerHistogramMODL::DiscretizeValues(ContinuousVector* cvSourceValues, IntVector* ivTargetIndexes,
						  int nTargetValueNumber, KWFrequencyTable*& kwftTarget) const
{
	...
	// On re-aguille si necessaire vers la nouvelle methode dediee represenattion vigule flottante
	if (GetHistogramSpec()->GetHistogramCriterion() == "G-Enum-fp")
	{
		MHTruncationDiscretizerHistogramMODL_fp discretizerHistogramMODL_fp;

		discretizerHistogramMODL_fp.GetHistogramSpec()->CopyFrom(GetHistogramSpec());
		discretizerHistogramMODL_fp.DiscretizeValues(cvSourceValues, ivTargetIndexes, nTargetValueNumber,
							     kwftTarget);
		return;
	}
	...
	if (GetHistogramSpec()->GetHistogramCriterion() == "G-Enum")
		cast(MHDiscretizerHistogramMODL*, this)->SetDiscretizationCosts(new MHMODLHistogramCosts);
	else if (GetHistogramSpec()->GetHistogramCriterion() == "Enum")
		cast(MHDiscretizerHistogramMODL*, this)->SetDiscretizationCosts(new MHEnumHistogramCosts);
	else if (GetHistogramSpec()->GetHistogramCriterion() == "KM")
		cast(MHDiscretizerHistogramMODL*, this)->SetDiscretizationCosts(new MHKMHistogramCosts);
	    ...
	    GetHistogramSpec()->GetHistogramCriterion() == "Enum")
	{
		...
		{
			dEpsilonBinNumber = MHHistogramSpec::GetMaxEpsilonBinNumber();
			...
			if (dEpsilonBinNumber > MHHistogramSpec::GetMaxEpsilonBinNumber())
				dEpsilonBinNumber = MHHistogramSpec::GetMaxEpsilonBinNumber();
...
MHHistogramSpec MHDiscretizerHistogramMODL::histogramSpec;

MHHistogramSpec* MHDiscretizerHistogramMODL::GetHistogramSpec()
{
	return &histogramSpec;
}

void MHDiscretizerHistogramMODL::ComputeHistogramFromBins(const ContinuousVector* cvSourceValues,
...
KWFrequencyTable* MHDiscretizerHistogramMODL::NewFrequencyTable() const
{
...
void MHDiscretizerHistogramMODL::MainDiscretizeValues(const ContinuousVector* cvSourceValues,
						      MHHistogram*& optimizedHistogram) const
{
	...
	if (GetHistogramSpec()->GetMaxIntervalNumber() > 0)
	{
		...
		optimizedHistogram = new MHHistogram;
		MainSimplifyHistogram(cvSourceValues, inputOptimizedHistogram, optimizedHistogram);
...
void MHDiscretizerHistogramMODL::MainSimplifyHistogram(const ContinuousVector* cvSourceValues,
						       const MHHistogram* optimizedHistogram,
						       MHHistogram*& simplifiedHistogram) const
{
	...
	Trace(sTmp + "Bin number\t" +
	      KWContinuous::ContinuousToString(optimizedHistogram->ComputeLargeScaleTotalBinLength()));
	Trace(sTmp + "Granularity\t" + LongintToString(optimizedHistogram->GetGranularity()));
	...
	else
	{
		...
		// partiles
		BuildGranularizedFrequencyTable(initialHistogramFrequencyTable, optimizedHistogram->GetGranularity(),
		...
		// standard
		nReferenceMaxIntervalNumber = GetMaxIntervalNumber();
		cast(MHDiscretizerHistogramMODL*, this)
		    ->SetMaxIntervalNumber(GetHistogramSpec()->GetMaxIntervalNumber());
		...
		cast(MHDiscretizerHistogramMODL*, this)->SetMaxIntervalNumber(nReferenceMaxIntervalNumber);
		...
		// On transforme le resultat en histogramme
		BuildOutputHistogram(cvSourceValues, 0, cvSourceValues->GetSize(),
				     optimizedGranularizedHistogramFrequencyTable, simplifiedHistogram);
...
void MHDiscretizerHistogramMODL::OutlierDiscretizeValues(const ContinuousVector* cvSourceValues,
							 MHHistogram*& optimizedHistogram) const
{
...
void MHDiscretizerHistogramMODL::TypicalDiscretizeValues(const ContinuousVector* cvSourceValues,
							 KWFrequencyTable*& optimizedHistogramFrequencyTable) const
{
	...
	if (GetHistogramSpec()->GetOptimalAlgorithm())
		cast(MHDiscretizerHistogramMODL*, this)->SetParam(Optimal);
	else
		cast(MHDiscretizerHistogramMODL*, this)->SetParam(OptimizedGreedyMerge);
	...
	if (GetHistogramSpec()->GetTruncationManagementHeuristic())
	{
		...
		if (bIsTruncationManagementHeuristicNecessary)
		{
			...
			nFrequency = cast(MHHistogramVector*, optimizedHistogramFrequencyTable->GetFrequencyVectorAt(0))
					 ->GetFrequency();
			assert(nFrequency > 0);
			...
			nFrequency = cast(MHHistogramVector*,
...
void MHDiscretizerHistogramMODL::StandardDiscretizeValues(const ContinuousVector* cvSourceValues,
							  KWFrequencyTable* initialHistogramFrequencyTable,
							  KWFrequencyTable*& optimizedHistogramFrequencyTable) const
{
	require(cvSourceValues != NULL);
...
void MHDiscretizerHistogramMODL::GranularizedDiscretizeValues(const ContinuousVector* cvSourceValues,
							      KWFrequencyTable* initialHistogramFrequencyTable,
							      KWFrequencyTable*& optimizedHistogramFrequencyTable) const
{
	...
	{
		// Arret si on depasse lniveau hierarchique max impose par l'utilisateur et qu'on est dans le mode
		// standard
		if (nPartileNumber > 1 and GetHistogramSpec()->GetMaxHierarchyLevel() > 0 and
		    nHierarchyLevel > GetHistogramSpec()->GetMaxHierarchyLevel() and
		    not GetHistogramSpec()->GetDeltaValues() and not GetHistogramSpec()->GetLogTrValues())
			break;
		nHierarchyLevel++;
...
void MHDiscretizerHistogramMODL::DiscretizeLogTrValues(const ContinuousVector* cvSourceValues,
						       KWFrequencyTable*& optimizedLogTrHistogramFrequencyTable) const
{
	ContinuousVector cvLogTrValues;
	MHHistogramSpec refHistogramSpec;
...
void MHDiscretizerHistogramMODL::SimplifyLogTrHistogram(const ContinuousVector* cvSourceValues,
							const KWFrequencyTable* optimizedLogTrHistogramFrequencyTable,
							ObjectArray* oaDataSubsets) const
{
	boolean bDisplay = false;
	int nOutlierEpsilonBinNumber;
	MHHistogramVector* histogramVector;
	...
	{
		histogramVector =
		    cast(MHHistogramVector*, optimizedLogTrHistogramFrequencyTable->GetFrequencyVectorAt(i));
...
void MHDiscretizerHistogramMODL::SplitPICHLogTrDataSubsets(const ContinuousVector* cvSourceValues,
							   ObjectArray* oaDataSubsets) const
{
	int i;
	MHDataSubset* dataSubset;
...
void MHDiscretizerHistogramMODL::SplitPICHLogTrDataSubset(const ContinuousVector* cvSourceValues,
...
void MHDiscretizerHistogramMODL::BuildSubHistograms(const ContinuousVector* cvSourceValues,
						    const ObjectArray* oaDataSubsets) const
{
	...
	for (i = 0; i < oaDataSubsets->GetSize(); i++)
	{
		...
		else
		{
			...
			subHistogramFrequencyTable->Initialize(1);
			histogramVector = cast(MHHistogramVector*, subHistogramFrequencyTable->GetFrequencyVectorAt(0));
...
void MHDiscretizerHistogramMODL::BuildAggregatedHistogram(const ContinuousVector* cvSourceValues,
							  const ObjectArray* oaDataSubsets,
							  MHHistogram*& aggregatedHistogram) const
{
	boolean bDisplay = false;
	MHHistogram* outputSubHistogram;
	MHHistogram* outputBoundaryHistogram;
	MHDataSubset* previousDataSubset;
	MHDataSubset* dataSubset;
	MHHistogramInterval* lastPreviousInterval;
	MHHistogramInterval* firstNextInterval;
	...
	MHHistogramVector* initialLastPreviousHistogramVector;
	...
	MHHistogramInterval* nextInterval;
	MHHistogramInterval* boundaryInterval;
	...
	aggregatedHistogram = new MHHistogram;

	// Creation des histogrammes de travail
	outputSubHistogram = new MHHistogram;
	outputBoundaryHistogram = new MHHistogram;
	...
	for (i = 0; i < oaDataSubsets->GetSize(); i++)
	{
		...
		{
			interval = outputSubHistogram->GetIntervalAt(n);
		...
		if (i > 0)
		{
			...
			firstNextInterval = outputSubHistogram->GetIntervalAt(0);
			...
			{
				interval = outputBoundaryHistogram->GetIntervalAt(n);
					 ...
					 nInitialBoundaryFirstIntervalFrequency)
				{
					...
					nextInterval = outputBoundaryHistogram->GetIntervalAt(n + 1);
			    ...
			    nBoundaryStartFrequency + nBoundaryIntervalFrequency < nBoundaryFrequency)
			{
				...
				boundaryInterval = boundaryInterval->Clone();
...
int MHDiscretizerHistogramMODL::BuildBoundaryHistogram(const ContinuousVector* cvSourceValues,
						       MHDataSubset* dataSubset1, MHDataSubset* dataSubset2) const
{
	...
	histogramVector = cast(MHHistogramVector*, histogramFrequencyTable->GetFrequencyVectorAt(
	...
	histogramVector = cast(MHHistogramVector*, histogramFrequencyTable->GetFrequencyVectorAt(0));
...
void MHDiscretizerHistogramMODL::ComputeBoundaryIntervalBounds(const ContinuousVector* cvSourceValues,
...
void MHDiscretizerHistogramMODL::DisplayDataSubsetsArray(const ALString& sTitle, ObjectArray* oaDataSubsets,
							 ostream& ost) const
{
	MHDataSubset* dataSubset;
	int i;
...
boolean MHDiscretizerHistogramMODL::IsDatasetPWCH(const ContinuousVector* cvSourceValues, int nTotalBinNumber) const
{
	return not IsDatasetPICH(cvSourceValues, nTotalBinNumber);
}

boolean MHDiscretizerHistogramMODL::IsDatasetPICH(const ContinuousVector* cvSourceValues, int nTotalBinNumber) const
{
	return IsDataSubsetPICH(cvSourceValues, 0, cvSourceValues->GetSize(), nTotalBinNumber);
}

boolean MHDiscretizerHistogramMODL::IsDataSubsetPWCH(const ContinuousVector* cvSourceValues, int nFirstIndex,
...
boolean MHDiscretizerHistogramMODL::IsDataSubsetPICH(const ContinuousVector* cvSourceValues, int nFirstIndex,
						     int nLastIndex, int nTotalBinNumber) const
{
	boolean bDisplay = false;
	boolean bIsPICH;
...
void MHDiscretizerHistogramMODL::ComputeDataLogTrParameters(const ContinuousVector* cvSourceValues, Continuous& cMinM,
...
Continuous MHDiscretizerHistogramMODL::LogTr(Continuous cX, Continuous cMinM, Continuous cMinP,
...
Continuous MHDiscretizerHistogramMODL::InvLogTr(Continuous cLogTrX, Continuous cMinM, Continuous cMinP,
						Continuous cMimDeltaLogM, Continuous cMimDeltaLogP) const
{
	Continuous cX;
...
void MHDiscretizerHistogramMODL::ComputeLogTrData(const ContinuousVector* cvSourceValues,
...
void MHDiscretizerHistogramMODL::ExportLogTrData(const ContinuousVector* cvSourceValues, const ALString& sFileName,
...
void MHDiscretizerHistogramMODL::ExportData(const ContinuousVector* cvSourceValues, const ALString& sFileName) const
{
	fstream fData;
	boolean bOk;
	int n;
...
void MHDiscretizerHistogramMODL::TruncationPostOptimization(
...
void MHDiscretizerHistogramMODL::DiscretizeDeltaValues(const ContinuousVector* cvSourceValues,
...
Continuous MHDiscretizerHistogramMODL::ComputeTruncationEpsilon(const ContinuousVector* cvSourceValues,
								const KWFrequencyTable* histogramFrequencyTable) const
{
	...
	if (bIsZeroVariationSingular)
	{
		...
		histogramVector = cast(MHHistogramVector*, histogramDeltaValueFrequencyTable->GetFrequencyVectorAt(0));
	...
	if (bIsZeroVariationSingular)
	{
		...
		histogramVector = cast(MHHistogramVector*, histogramDeltaValueFrequencyTable->GetFrequencyVectorAt(1));
	...
	if (bIsZeroVariationSingular)
	{
		...
		debug(histogramVector =
			  cast(MHHistogramVector*, histogramDeltaValueFrequencyTable->GetFrequencyVectorAt(1)));
	...
	if (bDisplay)
	{
		...
		{
			histogramVector =
			    cast(MHHistogramVector*, histogramDeltaValueFrequencyTable->GetFrequencyVectorAt(0));
...
int MHDiscretizerHistogramMODL::ComputeHistogramTotalFrequency(const KWFrequencyTable* histogramFrequencyTable) const
{
	int nTotalFrequency;
	MHHistogramVector* histogramVector;
	int n;
	...
	{
		histogramVector = cast(MHHistogramVector*, histogramFrequencyTable->GetFrequencyVectorAt(n));
		nTotalFrequency += histogramVector->GetFrequency();
	}
	return nTotalFrequency;
}

int MHDiscretizerHistogramMODL::ComputeHistogramTotalBinNumber(const KWFrequencyTable* histogramFrequencyTable) const
{
	int nTotalBinNumber;
	MHHistogramVector* histogramVector;
	int n;
...
int MHDiscretizerHistogramMODL::ComputeHistogramEmptyIntervalNumber(
    const KWFrequencyTable* histogramFrequencyTable) const
{
	int nEmptyIntervalNumber;
	MHHistogramVector* histogramVector;
...
int MHDiscretizerHistogramMODL::ComputeHistogramSingletonIntervalNumber(
...
int MHDiscretizerHistogramMODL::ComputeHistogramSingularIntervalNumber(
    const KWFrequencyTable* histogramFrequencyTable) const
{
	int nSingularIntervalBinLength;
...
boolean MHDiscretizerHistogramMODL::IsHistogramSingularInterval(const KWFrequencyTable* histogramFrequencyTable,
								int nIntervalIndex,
								int nSingularIntervalBinLength) const
{
	...
	{
		histogramVector =
		    cast(MHHistogramVector*, histogramFrequencyTable->GetFrequencyVectorAt(nIntervalIndex));
		bIsSingular =
	...
	{
		histogramVector =
		    cast(MHHistogramVector*, histogramFrequencyTable->GetFrequencyVectorAt(nIntervalIndex - 1));
...
int MHDiscretizerHistogramMODL::ComputeSingularIntervalBinLength(const KWFrequencyTable* histogramFrequencyTable) const
{
...
int MHDiscretizerHistogramMODL::ComputeLargestNonEmptyIntervalBinLength(
    const KWFrequencyTable* histogramFrequencyTable) const
{
	int nLargestNonEmptyIntervalBinLength;
	int n;
...
int MHDiscretizerHistogramMODL::ComputeBinIndex(Continuous cX, double dBinStart, double dEpsilonBinLength,
...
int MHDiscretizerHistogramMODL::ComputeNextBinIndex(Continuous cX, double dBinStart, double dEpsilonBinLength,
...
void MHDiscretizerHistogramMODL::ComputeInitialFrequencyTable(const ContinuousVector* cvSourceValues,
							      int nTotalBinNumber,
							      KWFrequencyTable*& initialHistogramFrequencyTable) const
{
	...
	initialHistogramFrequencyTable->Initialize(ivIntervalFrequencies.GetSize());
	for (n = 0; n < ivIntervalFrequencies.GetSize(); n++)
	{
		// Initialisation d'un intervalle de longueur 1 bin pour chaque valeur
		histogramVector = cast(MHHistogramVector*, initialHistogramFrequencyTable->GetFrequencyVectorAt(n));
...
void MHDiscretizerHistogramMODL::BuildGranularizedFrequencyTable(
    ...
    KWFrequencyTable*& granularizedHistogramFrequencyTable) const
{
	...
	MHHistogramVector* histogramVector;
	MHHistogramVector* granularizedHistogramVector;
	int nTotalFrequency;
	int nTotalLength;
	...
	granularizedHistogramFrequencyTable->Initialize(ivFilteredPartilesFrequencies.GetSize());
	for (n = 0; n < granularizedHistogramFrequencyTable->GetFrequencyVectorNumber(); n++)
	{
		granularizedHistogramVector =
		    cast(MHHistogramVector*, granularizedHistogramFrequencyTable->GetFrequencyVectorAt(n));
		...
		assert(n == 0 or granularizedHistogramVector->GetFrequency() > 0 or
		       cast(MHHistogramVector*, granularizedHistogramFrequencyTable->GetFrequencyVectorAt(n - 1))
			       ->GetFrequency() > 0);
	}
...
void MHDiscretizerHistogramMODL::WriteHistogramFrequencyTable(const ALString& sTitle,
...
void MHDiscretizerHistogramMODL::WriteHistogramFrequencyTableFile(const ALString& sTitle,
								  const ContinuousVector* cvSourceValues,
								  const KWFrequencyTable* histogramFrequencyTable,
...
void MHDiscretizerHistogramMODL::WriteHistogram(const ALString& sTitle, const MHHistogram* histogram,
						ostream& ost) const
{
...
void MHDiscretizerHistogramMODL::WriteHistogramFile(const ALString& sTitle, const MHHistogram* histogram,
						    const ALString& sFileName) const
{
	fstream fstOutput;
	boolean bOk;
...
void MHDiscretizerHistogramMODL::ExploitHistogram(const MHHistogram* histogram) const
{
	require(histogram != NULL);
...
void MHDiscretizerHistogramMODL::BuildOutputHistogram(const ContinuousVector* cvSourceValues, int nFirstIndex,
						      ...
						      MHHistogram* outputHistogram) const
{
	...
	MHHistogramVector nullHistogramVector;
	...
	nullHistogramVector.SetLength(cast(MHMODLHistogramCosts*, GetDiscretizationCosts())->GetTotalBinNumber());
	...
	if (GetHistogramSpec()->GetGranularizedModel())
		cast(MHMODLHistogramCosts*, GetDiscretizationCosts())->SetPartileNumber(1);
	...
	else
	{
		...
		for (n = 0; n < histogramFrequencyTable->GetFrequencyVectorNumber(); n++)
		{
			...
			assert(n == 0 or histogramVector->GetFrequency() > 0 or
			       cast(MHHistogramVector*, histogramFrequencyTable->GetFrequencyVectorAt(n - 1))
				       ->GetFrequency() > 0);
			...
			else
			{
				...
				// ComputeClosestUpperBound de MHHistogramSpec. Rappelons que les intervalles sont de la
				// forme ]LowerBound; UpperBound], et donc que LowerBound < LowerValue et UpperValue <=
...
void MHDiscretizerHistogramMODL::TraceOpen() const
{
...
void MHDiscretizerHistogramMODL::TraceClose() const
{
	if (bIsTraceOpened)
	{
...
void MHDiscretizerHistogramMODL::TraceBeginBlock(const ALString& sBlockName) const
{
	require(nTraceBlockLevel >= 0);
	if (bIsTraceOpened)
	{
...
void MHDiscretizerHistogramMODL::TraceEndBlock(const ALString& sBlockName) const
{
	if (bIsTraceOpened)
	{
		assert(nTraceBlockLevel >= 0);
...
void MHDiscretizerHistogramMODL::Trace(const ALString& sLine) const
{
	int i;
...
void MHDiscretizerHistogramMODL::PreparedCleanedStandardFrequencyTable(const MHHistogram* histogram,
								       int nMissingValueNumber,
								       KWFrequencyTable*& standardFrequencyTable) const
{
	boolean bDeleteEmptyIntervals = true;
	const MHHistogramInterval* sourceInterval;
	MHHistogramVector* targetVector;
	int n;
	...
	if (nMissingValueNumber > 0)
	{
		targetVector = cast(MHHistogramVector*, standardFrequencyTable->GetFrequencyVectorAt(0));
	...
	for (n = 0; n < histogram->GetIntervalNumber(); n++)
	{
		...
		{
			targetVector =
			    cast(MHHistogramVector*, standardFrequencyTable->GetFrequencyVectorAt(nIntervalIndex));
...
void MHDiscretizerHistogramMODL::InitializeFrequencyVector(KWFrequencyVector* kwfvFrequencyVector) const
{
	MHHistogramVector* frequencyVector;

	require(kwfvFrequencyVector != NULL);

	// Acces aux vecteurs dans le bon type
	frequencyVector = cast(MHHistogramVector*, kwfvFrequencyVector);
...
boolean MHDiscretizerHistogramMODL::CheckFrequencyVector(const KWFrequencyVector* kwfvFrequencyVector) const
{
	require(kwfvFrequencyVector != NULL);
	return true;
}

void MHDiscretizerHistogramMODL::InitializeWorkingData(const KWFrequencyTable* histogramFrequencyTable) const
{
	// On n'appelle pas la methode ancetre, qui se base sur des KWDenseFrequencyVector
	...
	cast(MHMODLHistogramCosts*, GetDiscretizationCosts())
...
void MHDiscretizerHistogramMODL::CleanWorkingData() const
{
	// On n'appelle pas la methode ancetre, qui se base sur des KWDenseFrequencyVector

	// Nettoyage du reste du parametrage
	GetDiscretizationCosts()->SetTotalInstanceNumber(0);
	cast(MHMODLHistogramCosts*, GetDiscretizationCosts())->SetTotalBinNumber(1);
	cast(MHMODLHistogramCosts*, GetDiscretizationCosts())->SetPartileNumber(0);
}

void MHDiscretizerHistogramMODL::AddFrequencyVector(KWFrequencyVector* kwfvSourceFrequencyVector,
						    const KWFrequencyVector* kwfvAddedFrequencyVector) const
{
	MHHistogramVector* sourcePartFrequencyVector;
	const MHHistogramVector* addedPartFrequencyVector;

	// Acces aux vecteurs dans le bon type
	sourcePartFrequencyVector = cast(MHHistogramVector*, kwfvSourceFrequencyVector);
	addedPartFrequencyVector = cast(const MHHistogramVector*, kwfvAddedFrequencyVector);
...
void MHDiscretizerHistogramMODL::RemoveFrequencyVector(KWFrequencyVector* kwfvSourceFrequencyVector,
						       const KWFrequencyVector* kwfvRemovedFrequencyVector) const
{
	MHHistogramVector* sourcePartFrequencyVector;
	const MHHistogramVector* removedPartFrequencyVector;

	// Acces aux vecteurs dans le bon type
	sourcePartFrequencyVector = cast(MHHistogramVector*, kwfvSourceFrequencyVector);
	removedPartFrequencyVector = cast(const MHHistogramVector*, kwfvRemovedFrequencyVector);
...
void MHDiscretizerHistogramMODL::MergeTwoFrequencyVectors(KWFrequencyVector* kwfvSourceFrequencyVector,
...
void MHDiscretizerHistogramMODL::MergeThreeFrequencyVectors(KWFrequencyVector* kwfvSourceFrequencyVector,
							    const KWFrequencyVector* kwfvMergedFrequencyVector1,
...
void MHDiscretizerHistogramMODL::SplitFrequencyVector(KWFrequencyVector* kwfvSourceFrequencyVector,
...
void MHDiscretizerHistogramMODL::MergeSplitFrequencyVectors(
...
void MHDiscretizerHistogramMODL::MergeMergeSplitFrequencyVectors(
