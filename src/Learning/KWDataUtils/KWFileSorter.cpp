...
// La taille maximale des chunks est determinee automatiquement. Les bornes de chaque chunks sont determinees
// en realisant un sample de la base (methode  KWFileSorter::SplitDatabase). Le calcul de la taille du sample
// est basee sur le papier de DeWitt 1991 "Parallel sorting on a shared-nothing architecture using probabilistic splitting"
// Ce calcul fait intervenir le parametre dSkew (deviation). Il represente la taille maximale / la moyenne des tailles,
// plus il est proche de 1 plus les chunks auront la taille attendue et plus l'echantillon necessaire sera grand.
// On a choisi dSkew=1.1, ceci permet d'avoir une bonne estimation de la distribution, le temps pour trier le sample n'est
