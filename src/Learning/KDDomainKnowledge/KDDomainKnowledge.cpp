...
KDDomainKnowledge::KDDomainKnowledge()
{
...
void KDDomainKnowledge::SetConstructionDomain(KDConstructionDomain* constructionDomainParam)
{
	constructionDomain = constructionDomainParam;
}

KDConstructionDomain* KDDomainKnowledge::GetConstructionDomain() const
{
	require(constructionDomain != NULL);
	return constructionDomain;
}

boolean KDDomainKnowledge::ComputeStats()
{
	int nLastRandomSeed;
...
void KDDomainKnowledge::RemoveConstructedClass()
{
	kwcConstructedClass = NULL;
	kwcdConstructedDomain = NULL;
}

void KDDomainKnowledge::DeleteConstructedClass()
{
...
boolean KDDomainKnowledge::IsClassConstructed() const
{
	require(IsStatsComputed());
	return kwcConstructedClass != NULL;
}

KWClass* KDDomainKnowledge::GetConstructedClass()
{
...
KWClassDomain* KDDomainKnowledge::GetConstructedDomain()
{
	// Verification a minima que la preparation a ete effectue
	require(IsStatsComputed());
	require(IsClassConstructed());
...
double KDDomainKnowledge::GetConstructionTime() const
{
	require(IsStatsComputed());
...
void KDDomainKnowledge::ComputeInitialAttributeCosts(KWClass* kwcValue) const
{
...
boolean KDDomainKnowledge::ImportAttributeMetaDataCosts(KWClass* kwcValue)
{
	boolean bOk = true;
	...
	while (attribute != NULL)
	{
		    ...
		    attribute->GetName() != GetTargetAttributeName())
		{
			...
			// Arret si probabilite totale des attribut superieure a 1
			if (dAllAttributeProb > 1 + 1e-6)
			{
				AddError("Sum a probabilities of all variables (originating from " +
...
int KDDomainKnowledge::RuleComputeUsedConstructionRuleNumber(const KWDerivationRule* constructedRule) const
{
...
int KDDomainKnowledge::RuleComputeUsedOperandNumber(const KWDerivationRule* constructedRule) const
{
	int nOperandNumber;
	int nOperand;
	KWDerivationRuleOperand* operand;
...
int KDDomainKnowledge::RuleComputeOperandTreeDepth(const KWDerivationRule* constructedRule) const
{
	int nMaxTreeDepth;
	int nTreeDepth;
	int nOperand;
...
int KDDomainKnowledge::GetMaxRuleNumber() const
{
	return nMaxRuleNumber;
}

void KDDomainKnowledge::SetMaxRuleNumber(int nValue)
{
	require(nValue >= 0);
	nMaxRuleNumber = nValue;
}

int KDDomainKnowledge::GetMaxRuleDepth() const
{
	return nMaxRuleDepth;
}

void KDDomainKnowledge::SetMaxRuleDepth(int nValue)
{
	require(nValue >= 0);
	nMaxRuleDepth = nValue;
}

double KDDomainKnowledge::GetMaxRuleCost() const
{
	return dMaxRuleCost;
}

void KDDomainKnowledge::SetMaxRuleCost(double dValue)
{
	require(dValue > 0);
	dMaxRuleCost = dValue;
}

void KDDomainKnowledge::Write(ostream& ost) const
{
	int i;
...
const ALString KDDomainKnowledge::GetClassLabel() const
{
	return "Knowledge domain";
}

const ALString KDDomainKnowledge::GetObjectLabel() const
{
	return GetClass()->GetName();
}

boolean KDDomainKnowledge::ConstructClass(KWClass*& constructedClass)
{
	boolean bOk = true;
	...
	selectionOperandAnalyser->SetDomainKnowledge(this);
	...
	if (bOk and oaAllConstructedRules.GetSize() > 0)
	{
		...
		classBuilder.SetDomainKnowledge(this);
...
void KDDomainKnowledge::DisplayConstructedRuleArray(const ObjectArray* oaConstructedRules, ostream& ost) const
{
	int nRule;
								...
								int nRuleNumber) const
{
	...
	oaAllConstructedRules->SetCompareFunction(KDConstructedRuleCompareCost);
...
void KDDomainKnowledge::BuildMainClassConstructedRules(ObjectArray* oaAllConstructedRules,
...
void KDDomainKnowledge::BuildAllConstructedRules(const KDConstructionRule* constructionRule,
...
void KDDomainKnowledge::BuildAllConstructedRulesFromLastOperands(
...
void KDDomainKnowledge::DispatchAttributeRandomDrawingNumbers(double dRandomDrawingNumber, int nAttributeNumber,
							      int nRuleNumber,
							      DoubleVector* dvAttributeRandomDrawingNumbers,
...
void KDDomainKnowledge::ComputeConstructionRuleProbs(double dRandomDrawingNumber, ObjectArray* oaConstructionRules,
...
void KDDomainKnowledge::DispatchConstructedRuleRandomDrawingNumbers(double dRandomDrawingNumber,
...
boolean KDDomainKnowledge::FilterConstructedRulesForRandomDrawing(double dRandomDrawingNumber,
...
boolean KDDomainKnowledge::IsConstructionBasicallyPossible(KWClass* mainClass) const
{
	int nRule;
	KDConstructionRule* constructionRule;
...
void KDDomainKnowledge::ComputeAllClassesCompliantRules(
...
KDClassDomainCompliantRules* KDDomainKnowledge::GetClassDomainCompliantRules() const
{
	return classDomainCompliantRules;
}

KDClassCompliantRules* KDDomainKnowledge::LookupClassCompliantRules(const ALString& sClassName) const
{
	return classDomainCompliantRules->LookupClassCompliantRules(sClassName);
}

boolean KDDomainKnowledge::IsConstructionRuleApplicable(const KDConstructionRule* constructionRule,
...
boolean KDDomainKnowledge::IsConstructionRuleApplicableFromLastOperands(
...
void KDDomainKnowledge::ExtractMatchingAttributes(const KDConstructionRule* constructionRule,
						  const KWDerivationRuleOperand* operand,
						  const KDClassCompliantRules* classCompliantRules,
...
void KDDomainKnowledge::ExtractMatchingRules(const KDConstructionRule* constructionRule,
					     const KWDerivationRuleOperand* operand,
					     const KDClassCompliantRules* classCompliantRules,
...
boolean KDDomainKnowledge::IsTypeMatching(int nRefType, const ALString& sRefObjectClassName, int nType,
...
boolean KDDomainKnowledge::IsTypeMatchingSelectionOperand(const KDConstructionRule* selectionRule,
...
void KDDomainKnowledge::BuildAllSelectionRulesFromSelectionOperand(
...
void KDDomainKnowledge::BuildAllSelectionParts(const KDClassCompliantRules* secondaryScopeClassCompliantRules,
					       double dSelectionRandomDrawingNumber, int nSelectionSize,
					       int nMaxSelectionOperandNumber, double dMaxSelectionCost,
					       const ALString& sPriorTreeNodeName,
					       const ObjectArray* oaSelectionOperands,
		...
		if (selectionOperandIndexFrequency->GetFrequency() > 0)
		{
			     ...
			     nCompleteOperand++)
			{
				...
				if (completeOperandIndexFrequency->GetFrequency() > 0)
				{
					...
					searchedSelectionPartition.SetClass(
					...
					if (foundSelectionPartition == NULL)
					{
						...
						foundSelectionPartition->SetClass(
						    searchedSelectionPartition.GetClass());
...
void KDDomainKnowledge::ComputeSelectionValues(const KDClassSelectionOperandStats* classSelectionOperandStats,
					       double dRandomDrawingNumber,
					       const KDConstructedPartitionDimension* partitionDimension,
...
boolean KDDomainKnowledge::IsSelectionRuleForbidden() const
{
	return bIsSelectionRuleForbidden;
}

void KDDomainKnowledge::SetSelectionRuleForbidden(boolean bValue) const
{
...
void KDDomainKnowledge::SortClassSelectionOperandStatsAndProbs(ObjectArray* oaClassSelectionOperandStats,
