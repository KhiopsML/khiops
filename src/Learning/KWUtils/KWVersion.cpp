...
{
	static boolean bIsInitialized = false;
	static boolean bSNBIgnoreDenseMissingValuesOnTrainMode = false;
	ALString sSNBIgnoreDenseMissingValuesOnTrainMode;

	// Determination du mode au premier appel
	if (not bIsInitialized)
	{
		// Recherche de la valeur de la variable d'environnement de l'option
		sSNBIgnoreDenseMissingValuesOnTrainMode = p_getenv("KhiopsSNBIgnoreDenseMissingValuesOnTrainMode");
		sSNBIgnoreDenseMissingValuesOnTrainMode.MakeLower();

		// Determination du mode
		if (sSNBIgnoreDenseMissingValuesOnTrainMode == "true")
			bSNBIgnoreDenseMissingValuesOnTrainMode = true;
		else if (sSNBIgnoreDenseMissingValuesOnTrainMode == "false")
			bSNBIgnoreDenseMissingValuesOnTrainMode = false;

		// DDD
		cout << "IgnoreDenseMissingOnTrain = " << (bSNBIgnoreDenseMissingValuesOnTrainMode ? "true" : "false")
		     << endl;

		// Memorisation du flag d'initialisation
		bIsInitialized = true;
	}
	return bSNBIgnoreDenseMissingValuesOnTrainMode;
}

boolean GetSNBIgnoreDenseMissingValuesOnDeployMode()
{
	static boolean bIsInitialized = false;
	static boolean bSNBIgnoreDenseMissingValuesOnDeployMode = false;
	ALString sSNBIgnoreDenseMissingValuesOnDeployMode;

	// Determination du mode au premier appel
	if (not bIsInitialized)
	{
		// Recherche de la valeur de la variable d'environnement de l'option
		sSNBIgnoreDenseMissingValuesOnDeployMode = p_getenv("KhiopsSNBIgnoreDenseMissingValuesOnDeployMode");
		sSNBIgnoreDenseMissingValuesOnDeployMode.MakeLower();

		// Determination du mode
		if (sSNBIgnoreDenseMissingValuesOnDeployMode == "true")
			bSNBIgnoreDenseMissingValuesOnDeployMode = true;
		else if (sSNBIgnoreDenseMissingValuesOnDeployMode == "false")
			bSNBIgnoreDenseMissingValuesOnDeployMode = false;

		// DDD
		cout << "IgnoreDenseMissingOnDeploy = " << (bSNBIgnoreDenseMissingValuesOnDeployMode ? "true" : "false")
		     << endl;

		// Memorisation du flag d'initialisation
		bIsInitialized = true;
	}
	return bSNBIgnoreDenseMissingValuesOnDeployMode;
}

boolean GetSNBDeploySparseBlocksWithMissingValues()
{
	static boolean bIsInitialized = false;
	static boolean bSNBDeploySparseBlocksWithMissingValues = false;
	ALString sSNBDeploySparseBlocksWithMissingValues;

	// Determination du mode au premier appel
	if (not bIsInitialized)
	{
		// Recherche de la valeur de la variable d'environnement de l'option
		sSNBDeploySparseBlocksWithMissingValues = p_getenv("KhiopsSNBDeploySparseBlocksWithMissingMode");
		sSNBDeploySparseBlocksWithMissingValues.MakeLower();

		// Determination du mode
		if (sSNBDeploySparseBlocksWithMissingValues == "true")
		{
			bSNBDeploySparseBlocksWithMissingValues = true;
			cout << "SNBDeploySparseBlocksWithMissing" << endl;
		}
		else if (sSNBDeploySparseBlocksWithMissingValues == "false")
			bSNBDeploySparseBlocksWithMissingValues = false;

		// Memorisation du flag d'initialisation
		bIsInitialized = true;
	}
	return bSNBDeploySparseBlocksWithMissingValues;
}
