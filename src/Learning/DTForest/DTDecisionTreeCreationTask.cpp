							     ...
							     ObjectArray* oaOutputAttributeStats)
{
	...
	if (learningSpec->GetTargetAttributeType() != KWType::Symbol)
	{
		if (GetForestParameter()->GetDecisionTreeParameter()->GetVerboseMode())
		...
		if (bOk)
		{
			...
			for (nAttribute = 0; nAttribute < nMaxCreatedAttributeNumberSlice; nAttribute++)
			{
				...
				if (ndTreeSingleton.Lookup(key) == NULL and dttree->GetTreeDepth() > 2)
				{
					...
					oatupletable.Add(BuildTupleTable(learningSpecTree, dttree, svariablename));
...
boolean DTDecisionTreeCreationTask::BluidForestAttributeSelections(DTForestAttributeSelection& forestAttributeSelection,
...
// void DTDecisionTreeCreationTask::SelectTreeAttributes(DTDecisionTree* tree, DTAttributeSelection* attgenerator,
// longint nObject)
//{
...
KWAttributeStats* DTDecisionTreeCreationTask::BuildAttributeStats(KWLearningSpec* learningSpec, DTDecisionTree* dttree,
...
KWTupleTable* DTDecisionTreeCreationTask::BuildTupleTable(KWLearningSpec* learningSpec, DTDecisionTree* dttree,
							  const ALString& svariablename) const
{
	// KWAttributeStats * attributeStats;
	KWTupleTable* univariateTupleTable;
...
int DTDecisionTreeCreationTask::ComputeMaxLoadableAttributeNumber(
    ...
    int nDecisionTreeMaxNumber, DTForestAttributeSelection& forestAttributeSelection) const
{
	...
	// ressoure memoire minimum pour avoir nNodeAttributesNumber atributs
	...
	{
		cout << "DTDecisionTreeCreationTask::ComputeMaxLoadableAttributeNumber" << endl;
...
longint DTDecisionTreeCreationTask::ComputeMeanStatMemory(ObjectDictionary* odInputAttributeStats) const
{
