...
public:
	...
	// Le maitre demande aux esclaves d'initailiser les ressources, puis appel de InitializeResourceSystem
...
protected:
	...
	friend class PLBufferedFileDriverRemote;
...
inline void PLMPITaskDriver::SendBlock(PLSerializer* serializer, PLMsgContext* context)
{
	...
	require(mpiContext->nMsgType == PLMPIMsgContext::SEND or mpiContext->nMsgType == PLMPIMsgContext::RSEND);
	check(serializer);
...
inline void PLMPITaskDriver::RecvBlock(PLSerializer* serializer, PLMsgContext* context)
{
	...
	mpiContext = cast(PLMPIMsgContext*, context);
	require(mpiContext->GetCommunicator() != MPI_COMM_NULL);
	require(mpiContext->nMsgType == PLMPIMsgContext::RECV);
	require(serializer->bIsOpenForRead);

	if (mpiContext->nMsgType != PLMPIMsgContext::BCAST)
	{
		tRecv.Start();
