...
MHTruncationDiscretizerHistogramMODL_fp::MHTruncationDiscretizerHistogramMODL_fp()
{
...
KWDiscretizer* MHTruncationDiscretizerHistogramMODL_fp::Create() const
{
	return new MHTruncationDiscretizerHistogramMODL_fp;
}

MHFloatingPointFrequencyTableBuilder* MHTruncationDiscretizerHistogramMODL_fp::NewFrequencyTableBuilder() const
{
...
MHTruncationDiscretizerHistogramMODL_fp::GetTruncationFrequencyTableBuilder() const
{
	return cast(MHTruncationFloatingPointFrequencyTableBuilder*, frequencyTableBuilder);
}

void MHTruncationDiscretizerHistogramMODL_fp::MainDiscretizeBins(const ContinuousVector* cvSourceBinLowerValues,
								 ...
								 ObjectArray* oaCoarsenedHistograms) const
{
	...
	if (not GetHistogramSpec()->GetTruncationManagementHeuristic())
		MHDiscretizerHistogramMODL_fp::MainDiscretizeBins(
		    cvSourceBinLowerValues, cvSourceBinUpperValues, ivSourceBinFrequencies, optimizedHistogram,
...
void MHTruncationDiscretizerHistogramMODL_fp::TruncationDiscretizeBins(const ContinuousVector* cvSourceBinLowerValues,
								       const ContinuousVector* cvSourceBinUpperValues,
								       const IntVector* ivSourceBinFrequencies,
								       MHHistogram*& optimizedHistogram,
								       MHHistogram*& postprocessedOptimizedHistogram,
								       ObjectArray* oaCoarsenedHistograms) const
{
	...
	MHDiscretizerHistogramMODL_fp::MainDiscretizeBins(cvSourceBinLowerValues, cvSourceBinUpperValues,
	...
	if (dTruncationEpsilon == 0)
	{
		...
		if (bIsTruncationNecessary)
		{
			...
			DiscretizeDeltaValues(cvSourceBinLowerValues, cvSourceBinUpperValues, ivSourceBinFrequencies,
					      cvSourceDeltaBinLowerValues, cvSourceDeltaBinUpperValues,
...
boolean MHTruncationDiscretizerHistogramMODL_fp::IsTruncationNecessary(const MHHistogram* optimizedHistogram) const
{
	boolean bIsTruncationNecessary;
	ALString sTmp;
...
void MHTruncationDiscretizerHistogramMODL_fp::DiscretizeDeltaValues(const ContinuousVector* cvSourceBinLowerValues,
								    const ContinuousVector* cvSourceBinUpperValues,
								    const IntVector* ivSourceBinFrequencies,
								    ContinuousVector*& cvSourceDeltaBinLowerValues,
								    ContinuousVector*& cvSourceDeltaBinUpperValues,
								    ...
								    MHHistogram*& optimizedDeltaValueHistogram) const
{
	...
	MHDiscretizerHistogramMODL_fp deltaValueDiscretizerHistogram;
	...
	GetHistogramSpec()->SetMaxCoarsenedHistogramNumber(0);
	GetHistogramSpec()->SetMaxHierarchyLevel(0);
	GetHistogramSpec()->SetMaxIntervalNumber(0);
...
double MHTruncationDiscretizerHistogramMODL_fp::ComputeTruncationEpsilon(
...
void MHTruncationDiscretizerHistogramMODL_fp::FinalizeHistogram(MHHistogram* outputHistogram) const
{
	...
	if (dTruncationEpsilon == 0)
		MHDiscretizerHistogramMODL_fp::FinalizeHistogram(outputHistogram);
	...
	else
	{
		...
		for (n = 0; n < outputHistogram->GetIntervalNumber(); n++)
		{
			...
			// Mais il peut y avoir des intervalles centraux dont la taille a ete reduit au point de rentrre
			// dans la singularite autour de 0
