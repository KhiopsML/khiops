...
//////////////////////////////////////////////////////////////////////////////////
// Groupeur de la famille MODL, avec gestion des cout de groupement de valeurs
class KWGrouperMODLFamily : public KWGrouper
{
public:
	// Indique que l'on fait partie de la famille MODL
	boolean IsMODLFamily() const override;

	// Cout de codage d'un groupage (definie par sa table d'effectifs et le nombre de valeurs descriptives groupees)
	virtual KWUnivariatePartitionCosts* GetGroupingCosts() const = 0;
	virtual double ComputeGroupingCost(KWFrequencyTable* kwftGroupedTable, int nInitialModalityNumber) const = 0;
	virtual double ComputeGroupingModelCost(KWFrequencyTable* kwftGroupedTable,
						int nInitialModalityNumber) const = 0;
	virtual double ComputeGroupingConstructionCost(KWFrequencyTable* kwftGroupedTable,
						       int nInitialModalityNumber) const = 0;
	virtual double ComputeGroupingPreparationCost(KWFrequencyTable* kwftGroupedTable,
						      int nInitialModalityNumber) const = 0;
	virtual double ComputeGroupingDataCost(KWFrequencyTable* kwftGroupedTable,
					       int nInitialModalityNumber) const = 0;
};

//////////////////////////////////////////////////////////////////////////////////
// Algorithme MODL de fusion des lignes quelconque d'une table de contingence
class KWGrouperMODL : public KWGrouperMODLFamily
{
public:
	// Constructeur
	KWGrouperMODL();
...
protected:
	...
	// groupes de valeurs
	void PostOptimizeGranularity(KWFrequencyTable* kwftTarget, IntVector* ivGroups,
...
inline KWMODLGroupMerge* KWGrouperMODL::GetGroupMergeAt(ObjectArray* oaGroupMerge, int nGroupNumber, int nIndex1,
							int nIndex2) const
{
...
inline void KWGrouperMODL::SetGroupMergeAt(ObjectArray* oaGroupMerge, int nGroupNumber, int nIndex1, int nIndex2,
					   KWMODLGroupMerge* groupMerge) const
{
	require(oaGroupMerge != NULL);
	require(nGroupNumber >= 0);
...
inline const KWFrequencyVector* KWGrouperMODL::GetFrequencyVectorCreator() const
{
	return groupingCosts->GetFrequencyVectorCreator();
}

inline double KWGrouperMODL::ComputePartitionCost(int nGroupNumber, int nGarbageModalityNumber) const
{
	return groupingCosts->ComputePartitionCost(nGroupNumber, nGarbageModalityNumber);
}

inline double KWGrouperMODL::ComputePartitionDeltaCost(int nGroupNumber, int nGarbageModalityNumber) const
{
	return groupingCosts->ComputePartitionDeltaCost(nGroupNumber, nGarbageModalityNumber);
}

inline double KWGrouperMODL::ComputeGroupCost(const KWFrequencyVector* groupFrequencyVector) const
{
	return groupingCosts->ComputePartCost(groupFrequencyVector);
}

inline double KWGrouperMODL::ComputePartitionGlobalCost(const KWFrequencyTable* partTable) const
{
	return groupingCosts->ComputePartitionGlobalCost(partTable);
}

inline int KWGrouperMODL::GetInitialValueNumber() const
{
