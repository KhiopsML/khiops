...
CommandFile::CommandFile()
{
	...
	sfoOutputCommands.SetBufferSize(1 * lMB);
	ResetParser();
...
boolean CommandFile::OpenOutputCommandFile()
{
	...
	// On ecrit a chaque flush ou endl si il le fichier est local
	if (FileService::GetURIScheme(sOutputCommandFileName) == "")
		sfoOutputCommands.SetFlushStandardMode(true);
...
ALString CommandFile::ReadNextCommandFileLine(InputBufferedFile* ibf, boolean& bLineTooLong) const
{
	...
	if (bOk)
	{
		...
		// La ligne est trop longue si elle depasee 8 Mo (contrainte de InputBufferedFile) ou si elle depasse nMaxLineLength
		bLineTooLong = bLineTooLong or cvLine.GetSize() > nMaxLineLength;
	}
...
boolean CommandFile::ReadInputCommand(StringVector* svIdentifierPath, ALString& sValue)
{
	...
	if (oaParserLoopLinesTokenTypes.GetSize() == 0)
	{
		...
		// C'est le seul cas qui ne peut pas etre traite par le parsing en flux des lignes du fichier de commande
