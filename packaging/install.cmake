...
if(UNIX)
  ...
  if(IS_CONDA)
    ...
    if(APPLE)
      # On macOS, we install in /usr/local/khiops/bin
      set(GET_PROC_NUMBER_PATH "/usr/local/khiops/bin/")
      set(MODL_PATH "/usr/local/khiops/bin/")
      set(USE_ENVIRONMENT_MODULE "")
    elseif(IS_FEDORA_LIKE)
      # On Fedora-like systems, we install MODL in the mpi bin directory and use environment modules
      ...
      # On standard Linux systems, we install in /usr/bin
      set(GET_PROC_NUMBER_PATH "/usr/bin/")
  ...
  elseif("${MPI_IMPL}" STREQUAL "mpich")
    ...
    if(APPLE)
      ...
      # Add environment variable to prevent deadlock with mpich (see issue https://github.com/pmodels/mpich/issues/6856)
      set(ADDITIONAL_ENV_VAR "export FI_PROVIDER=tcp # prevent deadlock with mpich on macOS")
      set(ADDITIONAL_ENV_VAR_DISPLAY "    echo FI_PROVIDER \"$FI_PROVIDER\"")
    endif(APPLE)
  endif()
  ...
  configure_file(${PROJECT_SOURCE_DIR}/packaging/linux/common/khiops.in ${TMP_DIR}/khiops_coclustering @ONLY
                 NEWLINE_STYLE UNIX)
  if(APPLE)
    install(TARGETS MODL MODL_Coclustering RUNTIME DESTINATION usr/local/khiops/bin COMPONENT KHIOPS_CORE)
    ...
    if(NOT "${MPI_IMPL}" STREQUAL "openmpi")
      ...
      install(TARGETS _khiopsgetprocnumber RUNTIME DESTINATION usr/local/khiops/bin COMPONENT KHIOPS_CORE)
    endif()
    ...
    if(NOT "${MPI_IMPL}" STREQUAL "openmpi")
      # Add _khiopsgetprocnumber to the khiops_core package except for openmpi
      install(TARGETS _khiopsgetprocnumber RUNTIME DESTINATION usr/bin COMPONENT KHIOPS_CORE)
    endif()
